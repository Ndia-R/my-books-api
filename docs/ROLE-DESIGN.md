...
このドキュメントは、`my-books` アプリケーションにおけるKeycloakのロール設計を定義するものです。

バックエンドエンジニア、フロントエンドエンジニア、Keycloak管理者から提出された以下の3つの設計案をレビューし、それぞれの優れた点を統合しました。
- `role-design-backend.md`
- `role-design-frontend.md`
- `role-design-keycloak.md`

**「なるべくシンプルにしたい」** というご要望を最優先しつつ、将来の機能拡張にも柔軟に対応できる、保守性と拡張性の高い設計を目指します。
...
本設計は、システムの成長に合わせて段階的に導入できます。

1.  **Phase 1 (最小構成)**: まずは `user` と `admin` の2つの役割のみを実装し、基本的な権限分離を実現します。
2.  **Phase 2 (有料化対応)**: サービスに有料プランを導入するタイミングで `premium-user` を追加します。
3.  **Phase 3 (運営体制強化)**: 運営チームが発足したら `content-editor` や `moderator` を追加し、権限を委譲します。

このアプローチにより、初期開発コストを抑えつつ、将来のスケールに備えることができます。